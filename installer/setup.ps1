...
# --- Torch installation ---
# cu128 supports Python 3.12 and 3.13 with CUDA 12.8
Log "INFO" "Installing torch>=2.6.0 (CUDA 12.8 wheel index, Python 3.13 compatible)..."
& $pip install "torch>=2.6.0" "torchaudio>=2.6.0" --index-url https://download.pytorch.org/whl/cu128 --quiet
if ($LASTEXITCODE -ne 0) {
    Log "WARN" "CUDA 12.8 wheel failed, trying CPU-only from PyPI..."
    & $pip install "torch>=2.6.0" "torchaudio>=2.6.0" --quiet
    if ($LASTEXITCODE -ne 0) {
        Log "ERROR" "Failed to install torch. Check your internet connection."
        exit 1
    }
}
Log "INFO" "torch installed successfully."
